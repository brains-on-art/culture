...
class Culture(object):
    def __init__(self):
        ...
        self.creature_parts[:, 2:] = 10.0  # Avoid undefined behavior by setting everything to one
    ...
    def add_jelly(self, index, position):
        ...
        if self.creature_data[index].alive == 1.0:
            self.remove_creature(index)

        cp = self.creature_physics[index]

        cp['target'] = pm.Body(5.0, 5.0)
        ...
        self.creature_data[index]['age'] = 0.0
        ...
        # self.creature_data[index]['type'] = 0 # so 0 == "jelly"
        # self.creature_data[index]['color'] = 0 # this is set below
